...
Use `.class{hero}`, `.class{roomy}`, `.class{dense}`, or
`.class{compact}` on `.slide{}` to tune typography scale while preserving the
...
.slide{.class{dense}
  ...
  .body{
    Dense slides keep more content readable without browser zoom changes.
  }
}
----

Set a deck-wide typography baseline in `.meta{}` when the whole presentation
needs larger or smaller text. Supported baselines are `compact`, `normal`,
`large`, `xlarge`, and `presentation`.
...
Use `scale` for precise control. When both `baseline` and `scale` are present,
`scale` wins.
...
Explicit column-group and column containers for side-by-side content.
...
* Use `.columns{}` as the explicit flex row wrapper.
...
* Blank lines inside `.columns{}` do not split the row

[NOTE]
====
Legacy adjacent `.column{}` blocks still compile side-by-side when no content
appears between them, but `.columns{}` is preferred because its layout contract
is explicit and robust to ordinary authoring whitespace.
====
...
| `.columns{}` | Effect | `<div class="columns">` | Explicit column group
