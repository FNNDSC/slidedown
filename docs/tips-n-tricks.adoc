...
The `.meta{}` directive supports comprehensive navbar customization, allowing you to create custom window-style navigation bars.

==== Basic Navbar Structure

The navbar customization system provides control over three areas:

* **Container**: The navbar wrapper (background, borders, padding)
* **Progress bar**: Optional slide progress indicator
* **Layout zones**: Left, center, and right positioning areas
...
Style the navbar container to create window titlebar effects:
...
* `shape` - "round", "square", or custom border-radius value
* `size` - Width and height (e.g., "24px", "32px")
* `background` - Background color or gradient
* `border` - Border styling
* `box-shadow` - Shadow effects for 3D appearance
* `color` - Icon/text color
* `icon` - FontAwesome HTML entity (e.g., "&#xf053") or empty string
* `margin` - All-sides margin
* `margin-right` - Right margin for spacing between buttons
* `tooltip` - Hover text

**Available button types:**

* `slide_first` - Jump to first slide
* `slide_previous` - Previous slide
* `slide_next` - Next slide
* `slide_last` - Jump to last slide
* `slide_counter` - Dynamic counter display
* `title` - Slide title display

==== Dynamic Slide Counter

Add a live-updating slide counter to the navbar:
...
The `{current}` and `{total}` placeholders are automatically replaced with actual slide numbers as you navigate.

==== Title Display

Display the current slide title in the navbar:
...
TIP: When using dark navbar backgrounds, set the title color to a light value for readability.
...
Arrange elements in three horizontal zones:

* `left` - Left-aligned elements
* `center` - Center-aligned elements
* `right` - Right-aligned elements

Elements are listed in order and rendered sequentially within their zone.
...
.meta{
  navbar:
    progress:
      show: true              # Show/hide the progress bar
      color: "#fb8500"        # Progress fill color
      background: "#4a4a4a"   # Track background color
      height: "3px"           # Bar height
}
----

Set `show: false` to completely hide the progress bar.

==== Disabling the Navbar

To hide the navbar entirely:
...
Use FontAwesome HTML entities for icons. Common ones:

* `&#xf053` - Left chevron (previous)
* `&#xf054` - Right chevron (next)
* `&#xf078` - Down chevron (first)
* `&#xf077` - Up chevron (last)
* `&#xf00d` - X (close)
* `&#xf068` - Minus (minimize)
* `&#xf0c8` - Square (maximize)

**Button Sizing:**

* Small controls: 20-24px
* Standard controls: 28-32px
* Large controls: 36-40px

**Color Coordination:**

Match navbar container background with title text color for readability:

* Dark backgrounds (#2d2d2d) → Light text (#f0f0f0)
* Light backgrounds (#e8e8e8) → Dark text (#333333)

**Round Button Centering:**

Round buttons automatically use flexbox centering for icons. No additional styling needed.
